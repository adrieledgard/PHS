        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              ...
              {{ Form::button('Edit', ['name'=>'btn_edit','class'=>'btn btn-warning btn-sm ','data-cat'=>$data->Id_type,'data-toggle'=>'modal','data-target'=>'#edit_modal']) }}
              {{ Form::button('Delete', ['name'=>'btn_delete','class'=>'btn btn-danger btn-sm ','onclick'=>'deletetype('.$data->Id_type.')']) }}
            </td>
          </tr>
          @endforeach
        </tbody>
    ...
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">Insert Type</h4>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <form class="" method='post' action='add_type'>
          @csrf
          <div class="modal-body">
            <div class="col-md-6">
              {{ Form::label('Type Name','') }}
              {{ Form::text('txt_type_name', '', ['class'=>'form-control']) }}
            </div>
          ...
          <div class="modal-footer">
            ...
            {{ Form::submit('Insert', ['name'=>'add_type', 'class'=>'btn btn-primary']) }}
          </div>
       </form>
        <div class="container-fluid">
  ...
  <div class="modal fade" id="edit_modal">
    <div class="modal-dialog modal-md">
      <div class="modal-content">
        ...
        <form class="" method='post' action='edit_type'>
          @csrf
          <div class="modal-body">
            <div class="col-md-6">
              {{ Form::hidden('id_type', '', ['class'=>'form-control','id'=>'id_type']) }}
              {{ Form::label('Type Name','') }}
              {{ Form::text('txt_type_name', '', ['class'=>'form-control','id'=>'txt_type_name']) }}
            </div>
            
          </div>
          <div class="modal-footer">
            {{ Form::button('Close', ['class'=>'btn btn-secondary','data-dismiss'=>'modal','aria-label'=>'Close']) }}
            {{ Form::submit('Edit', ['name'=>'edit_type', 'class'=>'btn btn-primary']) }}
          </div>
       </form>
        <div class="container-fluid">
...
<script>
  $(document).ready( function () {
    ...
    var myurl = "<?php echo URL::to('/'); ?>";
      ...
      {
        nama= arr[i]['Type_name'];
      }
      $("#id_type").val(id);
      $("#txt_type_name").val(nama);
      });
